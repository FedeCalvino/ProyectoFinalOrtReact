...
export const Ordenes = () => {
  ...
  const setPasoHecho = (ven, Paso) => {
    ...
    if (!ven || typeof ven !== "object") {
      console.error("Error: 'ven' no es un objeto válido", ven);
      return;
    }
    console.log("Paso",Paso)
    let CopiaVen = JSON.parse(JSON.stringify(ven));
  
    if (!Array.isArray(CopiaVen.ordenes)) {
      console.error("Error: 'ordenes' no es un array", CopiaVen.ordenes);
      return;
    }
  
    CopiaVen.ordenes.forEach((orden) => {
      orden.pasos.forEach((paso) => {
        if (paso.paso === Paso) {
          console.log("seteopaso")
    ...
    /*
    let CopiaLote = JSON.parse(JSON.stringify(lote));
  
    // 🔹 Reemplazar la venta en `ventas`
    CopiaLote.ventas = CopiaLote.ventas?.map((venta) => 
      venta.id === CopiaVen.id ? CopiaVen : venta
    );
            ...
            <div key={lote.idlote} className="lote-container">
              ...
              <div className="ordenes-container">
                {ven.ordenes.map((orden) => (
                  <OrdenCard key={orden.idOrden} orden={orden} cli={ven.cliente} className="orden-card" />
                ))}
              </div>
            </div>
          ))}
        ...
        <>
          ...
          {loading ? (
            ...
            <>
            {
              ...
              :
              <>
                <h2 className="ordenes">No hay Ordenes</h2>
              </>
            }
            </>
          )}
